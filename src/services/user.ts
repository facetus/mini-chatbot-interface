...
export async function friendInvite(id: number, friendId: number) {
    ...
    const isFriend = user.friends.findIndex(searchById(friend.id)) !== -1;
    const didInvite = user.invitations.findIndex(searchById(friend.id)) !== -1;
    const isInvited = friend.invites.findIndex(searchById(user.id)) !== -1;
...
export async function acceptInvite(id: string, friendId: string) {
    ...
    const isInvited = user.invites.findIndex(searchById(friend.id));
    const didInvite = friend.invitations.findIndex(searchById(user.id));
...
export async function deleteFriend(id: string, friendId: string) {
    ...
    const userIndex = user.friends.findIndex(searchById(friend.id));
    const friendIndex = friend.friends.findIndex(searchById(user.id));
